...
public:
    ...
    RaycastHit RaycastBlock(glm::vec3 origin, glm::vec3 direction, float maxDistance){
        ...
        //Punto di partenza
        int x = (int) std::floor(origin.x); //0
        int y = (int) std::floor(origin.y);
        int z = (int) std::floor(origin.z); //1

        //Direzione del passo
        //Esempio: Guardo verso l'alto Y > 0 , quindi aggiungiamo sempre +1 alla coordinata Y
        int stepX = direction.x > 0.0f ? 1 : -1; //1
        int stepY = direction.y > 0.0f ? 1 : -1; //-1
        int stepZ = direction.z > 0.0f ? 1 : -1; //1

        //Controlliamo se stiamo guardando esattamente un asse
        float tDeltaX = direction.x != 0.0f ? std::abs(1.0f / direction.x) : std::numeric_limits<float>::infinity(); //2
        float tDeltaY = direction.y != 0.0f ? std::abs(1.0f / direction.y) : std::numeric_limits<float>::infinity(); //inf
        float tDeltaZ = direction.z != 0.0f ? std::abs(1.0f / direction.z) : std::numeric_limits<float>::infinity(); //2


        float tMaxX = NextBoundary(origin.x, x, direction.x, stepX); //0.5, 0,0.5,1 => 1
        float tMaxY = NextBoundary(origin.y, y, direction.y, stepY); //inf
        float tMaxZ = NextBoundary(origin.z, z, direction.z, stepZ); //Origine in mondo 1,in blocco 1,0.5,1  => 2
